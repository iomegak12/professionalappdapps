...
APP_AGENT_ZIP=AppServerAgent-4.5.2.24246.zip
DB_AGENT_ZIP=dbagent-4.5.2.818.zip
MACHINE_AGENT_ZIP=MachineAgent-4.5.2.1611.zip
...
installAppServerAgent() {
  ...
  unzip -qo /agents/${APP_AGENT_ZIP} -d ${CATALINA_HOME}/appagent && rm /agents/${APP_AGENT_ZIP}
}

installDatabaseAgent() {
  echo "Installing Database Agent to ${DB_AGENT_HOME}..."
  unzip -qo /agents/${DB_AGENT_ZIP} -d ${DB_AGENT_HOME} && rm /agents/${DB_AGENT_ZIP}
}

installMachineAgent() {
  echo "Installing Machine Agent to ${MACHINE_AGENT_HOME}..."
  unzip -qo /agents/${MACHINE_AGENT_ZIP} -d ${MACHINE_AGENT_HOME} && rm /agents/${MACHINE_AGENT_ZIP}
}
...
setupAppdEnv() {
  ...
  echo export JAVA_AGENT_LOG_PATH="\"/tomcat/appagent/current/logs/${APPD_NODE_NAME}"\" >> /env.sh
